...
n_chars = lsum(data)
for i, x in enumerate(track(data, f"{Fore.WHITE}{Style.BRIGHT}encoding {Fore.WHITE}{Style.DIM}synth-100M{Style.RESET_ALL}")):
	data[i] = enc.encode(f"{x}<|eot|>\n", allowed_special="all")

n_toks = lsum(data)
print(f"{(n_chars/1e6)}M total chars,", f"{(n_toks/1e6)}M total tokens")

with open("data/synth-100M.bin", "wb") as f:
